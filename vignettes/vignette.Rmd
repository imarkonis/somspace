...
The Old World Drought Atlas [[Cook et al., 2016](link)] was used, as pre-processed by [Markonis et al. [2018]](link). The file can be downloaded [here](link).

```{r}
library(somspace)
load('../../owda_som/data/owda_for_som.rdata')
```

## Data preparation

First of all, we have to transform the raw data from `data.table` to `somin` class. 

```{r}
inp <- sominp(owda_raw[, 1:4])
```
